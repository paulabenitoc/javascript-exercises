    ...
    fetch(`${baseUrl}${name}`)
        ...
        .then(data => {
            ...
            const probability = (country.probability * 100).toFixed(2);
                return `<p id="${country.country_id}">El nombre ${name} tiene un ${probability}% de ser de ${country.country_id} <button onclick="deleteParagraph('${country.country_id}')">X</button></p>`;
        });
